import { Datum, TreeNode } from '../types';

export function sortChildrenWithSpouses(data: Datum[]): void {
  data.forEach(datum => {
...
export function toggleRels(tree_datum: TreeNode, hide_rels: boolean): void {
  ...
  function showHideChildren(): void {
    ...
    [tree_datum, ...spouses].forEach(sp =>
      children.forEach(ch_id => {
        if (sp.data[rels].children && sp.data[rels].children.includes(ch_id)) {
          if (!sp.data[rels_]) {
            sp.data[rels_] = {};
          }
          ...
          const index = sp.data[rels].children.indexOf(ch_id);
          if (index !== -1) {
...
export function toggleAllRels(tree_data: TreeNode[], hide_rels: boolean): void {
  tree_data.forEach(d => {
