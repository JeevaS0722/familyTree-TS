// src/CreateTree/formInfoSetup.ts
import * as icons from '../view/elements/Card.icons';
import { FormCreator } from './form';

/**
 * Creates the form UI and sets up event listeners
 *
 * @param form_creator - The form configuration object
 * @param closeCallback - Function to call when form is closed
 * @returns The form container element
 */
export function formInfoSetup(
  ...
  /**
   * Updates the form HTML and sets up event listeners
   */
  function update(): HTMLDivElement {
 ...
 */
function getHtml(form_creator: FormCreator): string {
  return ` 
  ...
  function addRelativeBtn(): string {
    return `
  ...
  function otherParentField(): string {
    if (!form_creator.other_parent_field) {
      return '';
    }
    return `
      <div class="f3-form-field">
        ...
        <select name="${form_creator.other_parent_field.id}">
          ${form_creator.other_parent_field.options
            .map(
              option => `
