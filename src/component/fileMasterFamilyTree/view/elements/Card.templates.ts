// src/view/elements/Card.templates.ts
...
interface CardTemplateProps {
  ...
  is_new?: boolean;
  image?: string | null;
  maleIcon?: ((props: { card_dim: CardDimensions }) => string) | null;
  femaleIcon?: ((props: { card_dim: CardDimensions }) => string) | null;
  x?: number;
  y?: number;
  rt?: number;
  closed?: boolean;
...
export function CardBody({
  ...
  card_dim,
  card_display,
}: CardTemplateProps): CardTemplateResult {
  return {
    template: `
      <g class="card-body">
...
export function CardText({
  ...
  if (Array.isArray(card_display)) {
    content = card_display
      .map(cd => `<tspan x="${0}" dy="${14}">${cd(d.data)}</tspan>`)
      .join('\n');
  } else if (card_display) {
    content = card_display(d.data);
  }
...
export function CardBodyAddNew({
  d,
  card_dim,
  card_add,
  label,
...
export function LinkBreakIconWrapper({
  ...
  let g = '',
    ...
    areParents = (r: any) => r.father || r.mother,
    areChildren = (r: any) => r.children && r.children.length > 0;
...
export function CardImage({
  ...
  function GenderlessIcon(): string {
    return `
