// src/CalculateTree/CalculateTree.ts
import d3 from '../d3';
...
import { Person, TreePerson, CalculatedTree, TreeDimensions } from '../types';
...
export default function CalculateTree({
  ...
  is_horizontal = false,
}: CalculateTreeParams): CalculatedTree {
  if (!data || !data.length) {
    return {
      data: [],
      data_stash: [],
  ...
  const data_stash: Person[] = single_parent_empty_card
  ...
  function calculateTreePositions(
    ...
    const root = d3.hierarchy<Person>(datum, hierarchyGetter);
    d3_tree(root);
    return root.descendants() as TreePerson[];

    function separation(
      a: d3.HierarchyNode<Person>,
      b: d3.HierarchyNode<Person>
    ): number {
      let offset = 1;
      if (!is_ancestry) {
        if (!sameParent(a, b)) {
    ...
    function hasCh(d: d3.HierarchyNode<Person>): boolean {
      return !!d.children;
    }
    ...
    function hasSpouses(d: d3.HierarchyNode<Person>): boolean {
    ...
    function hierarchyGetterChildren(d: Person): Person[] {
      return [...(d.rels.children || [])]
        .map(id => data_stash.find(d => d.id === id))
        .filter(Boolean) as Person[];
    }

    function hierarchyGetterParents(d: Person): Person[] {
  ...
  function nodePositioning({ tree }: { tree: TreePerson[] }): void {
    tree.forEach(d => {
  ...
  function setupSpouses({
    ...
    for (let i = tree.length; i--; ) {
      ...
      if (d.parents && d.parents.length === 2) {
        ...
        const x = (d: TreePerson, sp: TreePerson) =>
  ...
  function setupProgenyParentsPos({ tree }: { tree: TreePerson[] }): void {
    tree.forEach(d => {
      if (d.is_ancestry) {
        return;
      }
      ...
      function setupParentPos(d: TreePerson, p: TreePerson): void {
        d.psx = !is_horizontal ? p.sx : p.y;
        d.psy = !is_horizontal ? p.y : p.sx;
      }
    });

    function findDatum(id?: string): TreePerson | undefined {
      if (!id) {
        return undefined;
      }
  ...
  function setupChildrenAndParents({ tree }: { tree: TreePerson[] }): void {
    tree.forEach(d0 => {
      delete d0.children;
  ...
  function calculateTreeDim(
    ...
    if (!w_extent[0] || !w_extent[1] || !h_extent[0] || !h_extent[1]) {
      return { width: 0, height: 0, x_off: 0, y_off: 0 };
    }
  ...
  function createRelsToAdd(data: Person[]): Person[] {
    const to_add_spouses: Person[] = [];
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (d.rels.children && d.rels.children.length > 0) {
        ...
        d.rels.children.forEach(childId => {
          const child = data.find(d1 => d1.id === childId);
          if (!child) {
            return;
          }
