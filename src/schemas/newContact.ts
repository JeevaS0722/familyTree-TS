  ...
  const phoneSchema: Yup.Schema<PhoneSchemasData> = Yup.object().shape({
    areaCode: Yup.string()
      ...
      .test('phoneNoLength', t('phoneValidation'), function (value) {
        ...
        if (!prefix && !phoneNo && value === undefined) {
          return true;
        }
    ...
    prefix: Yup.string()
      ...
      .test('phoneNoLength', t('phoneValidation'), function (value) {
        ...
        if (!areaCode && !phoneNo && value === undefined) {
          return true;
        }
      }) as Yup.StringSchema<string>,
    phoneNo: Yup.string()
      ...
      .test('phoneNoLength', t('phoneValidation'), function (value) {
        ...
        if (areaCode && prefix && (value === undefined || value.length === 4)) {
          return true;
        }

        if (!prefix && !areaCode && value === undefined) {
          return true;
        }
      }) as Yup.StringSchema<string>,
  });

  const emailSchema: Yup.Schema<EmailSchemaData> = Yup.object().shape({
    email: Yup.string()
      .trim()
  ...
  const altNameSchema: Yup.Schema<AltNameSchemaData> = Yup.object().shape({
    altName: Yup.string()
      ...
      .test('altNameValidation', t('altNameValidation'), function (value) {
        const { firstName = '', lastName = '' } = this.options.context || {};
        if (!value?.trim()) {
          return true;
        } // Allow empty AltName
