...
import { useAddContactNoteMutation } from '../../../store/Services/noteService';
import { noteSchema } from '../../../schemas/note';
import { contactTaskType } from '../../../utils/constants';
...
const AddNotes: React.FC = () => {
  ...
  const { contactId } = (location.state as LocationStateData) || {};
  ...
  const onSubmit = async (values: addDeedOrContactNoteValues) => {
    try {
      const data: addContactNotePayload = {
        contactId,
        type: values.type,
        memo: values.memo,
        fileStatus: values.fileStatus,
      };
      const response = await addContactNote(data);
      if ('data' in response) {
        if (response?.data?.success) {
          ...
          navigate(`/editContact/${contactId}?tab=notes`);
        }
      }
    } catch (error) {
      dispatch(
